...
#!
def compute_correlation(y_pred1, y_pred2):
    y_pred1 = y_pred1 - y_pred1.mean()
    y_pred2 = y_pred2 - y_pred2.mean()
    cov = (y_pred1 * y_pred2).mean()
    std1 = y_pred1.std() + 1e-8
    std2 = y_pred2.std() + 1e-8
...
class PrefARMTrainer(DPOTrainer):
    ...
    #!
    def _inject_preference(self, model, preference):
